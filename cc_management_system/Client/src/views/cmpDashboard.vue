...
export default {
  ...
  data() {
    return {
      ...
      showDropdown: false,
      statsData: [],
      selectedDates: [], // For date range selection
      datePickerConfig: {
        mode: 'range',
        dateFormat: 'Y-m-d', 
        ...
        altInput: true, 
        altFormat: 'Y-m-d', 
        altInputClass: 'flatpickr-custom', 
      },
    };
  },
  computed: {
  ...
  methods: {
    ...
    async fetchStatsData() {
      try {
        ...
        const response = await fetch(`http://localhost:3500/api/allStats?&start=${startDate}&end=${endDate}`);
        const responseData = await response.json();

        console.log("ResponseData: ", responseData);

        const plotData = responseData.allStatsWithinRange.map(day => ({
          ...
          numberOfCustomers: day.numberOfCustomers
        }));

        this.statsData = plotData;
        this.createOptinRatePlot(plotData);
        this.createCustomerTimelinePlot(plotData);
    ...
    createOptinRatePlot(data) {
      ...
      const plotData = [{
        ...
        hovertemplate: '<b>%{x}</b><br>' +
                       'Zustimmungsrate: %{y}%<br>' +
                       'OptIn: %{customdata[0]}<br>' +
                       'OptOut: %{customdata[1]}<br>' +
                       'Total: %{customdata[2]}',
        customdata: data.map(day => [day.optIn, day.optOut, day.total]),
        line: { color: '#662D8D' }, 
      }];
    ...
    createCustomerTimelinePlot(data) {
      const dates = data.map(day => day.datum);
      const customerCounts = data.map(day => day.numberOfCustomers);
      ...
      const layout = {
        title: 'Nutzende Kunden über den Zeitraum',
        xaxis: {
          title: 'Datum',
        },
        yaxis: {
      ...
      const config = { responsive: true };
      Plotly.newPlot(this.$refs.plot1, plotData, layout, config);
    },
...
.pie-chart {
  width: 30%; /* Breite der Kreisdiagramme */
  margin: 0 auto;
}
